...
// Preloader removed — no DOM or blocking behavior.
...
if (hamburger && mobileMenu) {
  ...
  window.addEventListener('resize', () => {
    if (window.innerWidth > 880 && mobileMenu.style.display !== 'none') {
...
/* ===== App-like enhancements: FAB and smooth navigation ===== */
try{
  ...
  if (fab) {
    fab.addEventListener('click', () => {
      if (!fabMenu) return;
      const isOpen = fabMenu.style.display === 'flex' || fabMenu.style.display === 'block';
      fabMenu.style.display = isOpen ? 'none' : 'flex';
      // subtle pulse
      fab.animate([{transform:'translateY(0)'},{transform:'translateY(-6px)'},{transform:'translateY(0)'}],{duration:260});
    });
  }

  if (fabContact) fabContact.addEventListener('click', (e)=>{
    e.preventDefault();
    // prefer modal contact if available
    if (typeof openModal === 'function') { openModal(contactHtml()); attachModalFormHandlers(); }
    else window.location.href = 'Contact.html';
  });

  if (fabDonate) fabDonate.addEventListener('click', (e)=>{
    e.preventDefault();
    // Lead user to partner page
    pageTransition && pageTransition.classList.add('show');
    setTimeout(()=> window.location.href = 'Partner.html', 240);
  });
  ...
  window.addEventListener('scroll', () => {
    ...
    if (y > lastY && y > 120) fab.style.transform = 'translateY(80px)';
    else fab.style.transform = 'translateY(0)';
    lastY = y;
  }, {passive:true});
}catch(e){console.error('FAB init error',e)}

// smooth scrolling for in-page anchors (ensure offset for sticky header)
